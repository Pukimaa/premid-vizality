...
const error = (s) =>
  console.warn(`%c[PreMiD]%c ${s}`, log_prefix, "font-weight: bold");
const success = (s) =>
  console.log(`%c[PreMiD]%c ${s}`, log_prefix, "font-weight: bold");
...
const setActivity = async (rpc) => {
  ...
  if (applications[rpc.clientId]) name = applications[rpc.clientId];
    ...
    const data = await get({
      url: `/applications/${rpc.clientId}/public?with_guild=false`,
    });
  ...
  if (!activity.assets?.large_text)
    activity.assets = { large_text: `PreMiD Vizality ${version}` };
  else
    activity.assets.large_text = `PreMiD Vizality ${version} • Ext ${
      activity.assets.large_text.split("Ext ")[1]
    }`;
...
function socketError(e) {
  ...
  if (e.code === "EADDRINUSE") {
    vizality.api.notices.sendToast(
      `premid-boundPort-${Math.floor(Math.random() * 200)}`,
      {
        header: "PreMiD Websocket Port Already Bound",
        content:
          "Cause: PreMiD App/Dameon still installed/running, residual Discord background processes running from crashes, etc.",
        timeout: 3000,
      }
    );
  }
